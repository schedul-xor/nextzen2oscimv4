...
#        print(lon,lat,lon3857,lat3857)
    ...
    for layer in layers:
        ...
        for feature in features:
            ...
            if 'type' in kv:
                type_ = kv['type']
                if 'class' not in kv and type_ in frozenset(['yes','office','apartments','house','residential','commercial','hotel','parking','school','hospital','retail']) and geometry_type == 'Polygon':
                    fixed_kv['building'] = 'yes'

                    if 'height' in kv:
                        fixed_kv['height'] = str(int(kv['height'])*200)
